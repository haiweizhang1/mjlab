...
export WARP_CACHE_PATH="${WARP_CACHE_PATH:-/tmp/mjlab-warp-depth-legacy512-action-only}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/mjlab-mpl-depth-legacy512-action-only}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/mjlab-xdg-depth-legacy512-action-only}"
export PYTORCH_ALLOC_CONF=expandable_segments:True
export MUJOCO_GL="${MUJOCO_GL:-egl}"
export PYOPENGL_PLATFORM="${PYOPENGL_PLATFORM:-egl}"

exec "$train_entrypoint" \
  Mjlab-Velocity-Football-Depth-KlavierLegacy512Noise0Teacher-PushCurrOff-FrozenMLP-NoSym-ActionOnlyDistillation-Flat-Unitree-G1 \
  ...
  --agent.run-name DepthStudent_KlavierLegacy512_Noise0Teacher50000_FrozenMLP_TeacherRollout_ActionHuberOnly_NoDelay_MountRange025_seed42_10k_wandb
