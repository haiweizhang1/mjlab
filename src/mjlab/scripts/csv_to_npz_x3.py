...
from mjlab.tasks.tracking.config.x3.env_cfgs import droid_x3_flat_tracking_env_cfg
    ...
    while not file_saved:
        ...
        if not file_saved:
            ...
            if reset_flag and not file_saved:
                ...
                print("\nStacking arrays and saving local data...")
                # 堆叠日志数组
                for k in (
                        "joint_pos",
                        "joint_vel",
                ...
                # 本地保存npz文件，使用output_name作为文件名（自动补全.npz后缀）
                save_path = f"npz_mjlab_x3/{output_name}.npz" if not output_name.endswith(".npz") else output_name
                np.savez(save_path, **log)
                print(f"✅ 运动数据已本地保存至: {save_path}")

                # 若开启渲染，本地生成mp4视频（无wandb上传）
                if render and frames:
                    from moviepy import ImageSequenceClip
                    print("📹 开始生成本地视频...")
                    video_path = f"npz_mjlab_x3/{output_name}.mp4" if not output_name.endswith(".mp4") else output_name
                    clip = ImageSequenceClip(frames, fps=output_fps)
                    # 加入logger=None抑制moviepy的冗余日志，提高体验
                    clip.write_videofile(video_path, logger=None)
                    print(f"✅ 渲染视频已本地保存至: {video_path}")
...
def main(
    ...
    """Replay motion from CSV file and output to local npz file (no wandb).

    Args:
        input_file: Path to the input CSV file.
        output_name: Name of the local output file (without suffix, auto add .npz/.mp4).
        input_fps: Frame rate of the CSV file.
        output_fps: Desired output frame rate.
        device: Device to use (cuda:0/cpu).
        render: Whether to render the simulation and save local mp4 video.
    ...
    scene = Scene(droid_x3_flat_tracking_env_cfg().scene, device=device)
    model = scene.compile()
