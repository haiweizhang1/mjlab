...
- `main`：官方 `upstream/main` 的镜像，只接收上游快进更新，不放足球项目修改。
- `soccer/stable`：已经由项目负责人确认、验证且可复现的足球项目主线。
- `soccer/train`：默认工作分支，用于任务配置、训练方法和阶段性结构调整。
...
日常修改默认从 `soccer/train` 开始。评审时必须以 `BASELINES.md` 指定的正式基线
和当前最新任务配置为两个明确比较对象，展示配置差异、指标、验证条件和已知风险。
只有项目负责人确认后，才将选定提交通过 `cherry-pick` 或受控合并晋升到
`soccer/stable`；不得因单次训练回报较高而静默替换正式基线。

上游更新按 `upstream/main -> main -> soccer/stable -> soccer/train` 的方向引入。
未经兼容性检查和项目负责人确认，不把新上游提交直接合入两个足球分支。
